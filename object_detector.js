...
function prepare_input(img) {
    ...
    canvas.width = 640;  // Keep the input size for the YOLO model
    canvas.height = 640;
    const context = canvas.getContext("2d");
...
function process_output(output, img_width, img_height) {
    ...
    for (let index = 0; index < 8400; index++) {
        ...
        const x1 = (xc - w / 2) / 640 * img_width;  // Adjust calculation for larger display size
        const y1 = (yc - h / 2) / 640 * img_height;  // Adjust calculation for larger display size
        const x2 = (xc + w / 2) / 640 * img_width;  // Adjust calculation for larger display size
        const y2 = (yc + h / 2) / 640 * img_height;  // Adjust calculation for larger display size
...
function draw_boxes(canvas, boxes) {
    ...
    boxes.forEach(([x1, y1, x2, y2, label]) => {
        ...
        link.style.zIndex = "10"; // Ensure the link is on top of the canvas
        link.style.border = "2px solid rgba(0, 255, 0, 0.5)";

        // Append the link to the canvas' parent element
